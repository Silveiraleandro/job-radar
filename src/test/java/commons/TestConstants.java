package commons;

public class TestConstants {
    public static final String JAVA_DEV = "Java Developer";
    public static final String TEST_COMPANY = "TestCompany";
    public static final String VANCOUVER = "Vancouver";
    public static final String URL = "https://example.com";
    public static final String URL_JOBS_1 = "https://example.com/jobs/1";
    public static final String ACCOUNT_EXC_AI_SALES = "Account Executive, AI Sales";
    public static final String SAN_FRAN = "San Francisco, CA";
    public static final String STRIPE_URL = "https://stripe.com/jobs/search?gh_jid=example";
    public static final String SPRING_BOOT_ENGINEER = "Spring Boot Engineer";
    public static final String MOCK_COMPANY = "Mock Company";
    public static final String REMOTE = "Remote";
}
